...
class EquipmentProtocol(models.Model):
    ...
    note_rel =fields.Html(related='mail_activity_id.note',string='Anmerkung')
    folg_beg_ids = fields.Many2many('folgebegehung', string='Folgebegehung', store=True)
    folg_beg_id = fields.Many2one('folgebegehung', store=True)
    gefaehrdunsfaktor_ids = fields.Many2many('equipment.types', string='Gefahrenfaktor', store=True)
    gefaehrdunsfaktor_betriebsanweisun_ids = fields.Many2many('equipment.types', 'protocol_equipment_type_rel', 'protocol_id', 'equipment_type_id', string='Gefahrenfakt.')
    gefaehrdunsfaktor_id = fields.Many2one('equipment.types', store=True)
