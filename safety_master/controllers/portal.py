...
class CustomerPortal(CustomerPortal):
    ...
    def prepare_portal_values(self, global_search=None):
        ...
        equipments = request.env['safety.master'].search(equipment_domain)
        protocol_domain += [('equipment_id', 'in', equipments.ids)]
        values['equipment_count'] = request.env['safety.master'].search_count(equipment_domain)
        values['equipments'] = equipments
        values['protocol_count'] = request.env['hazard.protocol'].search_count(protocol_domain)
        return values
    ...
    def portal_my_equipments(self, page=1, date_begin=None, date_end=None, sortby=None, filterby=None, search=None, search_in='equipment', groupby='none', global_search=False, **kw):
        ...
        Equipment = request.env['safety.master']
        searchbar_sortings = {
...
#         archive_groups = self._get_archive_groups('safety.master', domain)
...
#             'archive_groups': archive_groups,
        ...
        return request.render("safety_master.portal_my_equipments", values)
    ...
    def portal_my_equipment(self, equipment_id, access_token=None, **kw):
        try:
            equipment_sudo = self._document_check_access('safety.master', equipment_id, access_token)
        except (AccessError, MissingError):
            return request.redirect('/my')
        equipment_protocols = request.env['hazard.protocol'].search([('equipment_id', '=', equipment_id)])
        res_model_id = request.env['ir.model'].sudo().search([('model', '=', 'safety.master')], limit=1).id
        ...
        return request.render("safety_master.portal_my_equipment", values)
    ...
    def portal_my_protocols(self, page=1, date_begin=None, date_end=None, sortby=None, filterby=None, search=None, search_in='all', groupby='none', global_search=False, **kw):
        values = self.prepare_portal_values(global_search)
        Protocol = request.env['hazard.protocol']
...
# archive groups - Default Group By 'create_date'
#         archive_groups = self._get_archive_groups('hazard.protocol', domain)
...
#             'archive_groups': archive_groups,
        ...
        return request.render("safety_master.portal_my_protocols", values)
    ...
    def portal_my_protocol(self, protocol_id, access_token=None, **kw):
        try:
            protocol_sudo = self._document_check_access('hazard.protocol', protocol_id, access_token)
        except (AccessError, MissingError):
            return request.redirect('/my')

        values = self._protocol_get_page_view_values(protocol_sudo, access_token, **kw)
        return request.render("safety_master.portal_my_protocol", values)
        ...
        try:
            ...
            for activity_id in all_activities:
                ...
                if activity_id.res_model != "maintenance.request":
                    ...
                    equipment = _("Equipment : ") + str(safety.master_id.name) if safety.master_id else ""
    ...
    def print_protocol_report(self, protocol_id, **kw):
        ...
        if kw['type'] == 'maintenance':
            pdf = request.env.ref('safety_master.safety_master_protocol_maintenance').sudo()._render_qweb_pdf([protocol_id])[0]
#         elif kw['type'] == 'prot':
#             pdf = request.env.ref('safety_master.safety_master_protocol_rep_prot').sudo()._render_qweb_pdf([protocol_id])[0]
#         elif kw['type'] == 'protocol':
#             pdf = request.env.ref('safety_master.safety_master_protocol').sudo()._render_qweb_pdf([protocol_id])[0]
        elif kw['type'] == 'calibration':
            pdf = request.env.ref('safety_master.gefaehrdungsbeurteilungs_protocol').sudo()._render_qweb_pdf([protocol_id])[0]#gefährdungsbeurteilung
        elif kw['type'] == 'routine_test':
            pdf = request.env.ref('safety_master.safety_master_eichnachweis_protocol').sudo()._render_qweb_pdf([protocol_id])[0]#Begehung
        elif kw['type'] == 'el_test':
            pdf = request.env.ref('safety_master.folgebegehungs_protocol').sudo()._render_qweb_pdf([protocol_id])[0]#Folgebegehungsprotokoll
        elif kw['type'] == 'unterweisung':
            pdf = request.env.ref('safety_master.unterweisungs_protocol').sudo()._render_qweb_pdf([protocol_id])[0]#Unterweisung
        elif kw['type'] == 'gefahrstoff_verszeichnis':
            pdf = request.env.ref('safety_master.safety_master_gef_verzeichnis_protocol').sudo()._render_qweb_pdf([protocol_id])[0]#Gefahrstoff-Verzeichnis
        elif kw['type'] == 'betriebsanweisung':
            pdf = request.env.ref('safety_master.betriebsanweisung_protocol').sudo()._render_qweb_pdf([protocol_id])[0]#Betriebsanweisung Maschinen und Arbeitsverfahren
        elif kw['type'] == 'calibration':
            pdf = request.env.ref('safety_master.safety_master_betriebsanweisung_gefahrstoffe').sudo()._render_qweb_pdf([protocol_id])[0]#Betriebsanweisung Gefahrstoffe
        elif kw['type'] == 'maintenance':
            pdf = request.env.ref('safety_master.safety_master_betriebsanweisung_psa').sudo()._render_qweb_pdf([protocol_id])[0]#Betriebsanweisung PSA
        protocol = request.env['hazard.protocol'].browse(int(protocol_id))
        filename = ''
        if protocol:
    ...
    def print_report_protocol(self, **kw):
        ...
        if protocols:
            ...
            protocols = request.env["hazard.protocol"].search([('id', 'in', protocols)])
            pdf = False
            stream = io.BytesIO()
            with zipfile.ZipFile(stream, 'w') as archive:
                for protocol in protocols:
                    if protocol.equipment_test_type == 'el_test' and protocol.mail_activity_id:
                        pdf = request.env.ref('safety_master.folgebegehungs_protocol').sudo()._render_qweb_pdf([protocol.id])[0]
                    elif protocol.equipment_test_type == 'calibration_ei' and protocol.mail_activity_id:
                        pdf = request.env.ref('safety_master.gefaehrdungsbeurteilungs_protocol').sudo()._render_qweb_pdf([protocol.id])[0]
                    elif protocol.equipment_test_type == 'routine_test' and protocol.mail_activity_id:
                        pdf = request.env.ref('safety_master.safety_master_eichnachweis_protocol').sudo()._render_qweb_pdf([protocol.id])[0]
                    elif protocol.equipment_test_type == 'betriebsanweisung' and protocol.mail_activity_id:
                        pdf = request.env.ref('safety_master.betriebsanweisung_protocol').sudo()._render_qweb_pdf([protocol.id])[0]
                    elif protocol.equipment_test_type == 'unterweisung' and protocol.mail_activity_id:
                        pdf = request.env.ref('safety_master.unterweisungs_protocol').sudo()._render_qweb_pdf([protocol.id])[0]
                    elif protocol.equipment_test_type == 'calibration' and protocol.mail_activity_id:
                        pdf = request.env.ref('safety_master.safety_master_betriebsanweisung_gefahrstoffe').sudo()._render_qweb_pdf([protocol.id])[0]
                    elif protocol.equipment_test_type == 'maintenance' and protocol.mail_activity_id:
                        pdf = request.env.ref('safety_master.safety_master_betriebsanweisung_psa').sudo()._render_qweb_pdf([protocol.id])[0]
#                     elif protocol.equipment_test_type == 'uvv' and protocol.category_id == request.env.ref("safety_master.equipment_hebebuhne"):
#                         pdf = request.env.ref('safety_master.safety_master_hebebuhne_protocol').sudo()._render_qweb_pdf([protocol.id])[0]
#                     elif protocol.equipment_test_type == 'uvv' and protocol.category_id  == request.env.ref("safety_master.equipment_tore"):
#                         pdf = request.env.ref('safety_master.safety_master_tore_protocol').sudo()._render_qweb_pdf([protocol.id])[0]
#                     elif protocol.maintenance_request_id:
#                         pdf = request.env.ref('safety_master.safety_master_protocol_rep_prot').sudo()._render_qweb_pdf([protocol.id])[0]
#                     elif protocol.equipment_test_type == 'maintenance' and protocol.mail_activity_id:
#                         pdf = request.env.ref('safety_master.safety_master_protocol_maintenance').sudo()._render_qweb_pdf([protocol.id])[0]
                    if pdf:
                        if protocol:
                            protocol.downloaded_user_ids = [(4, request._uid)]
