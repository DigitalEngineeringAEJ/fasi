...
class HazardMailActivity(models.AbstractModel):
    _name = 'hazard.mail.activity'
    _description = 'Hazard Mail Activity'

    signature = fields.Binary(string='Signature')
    type = fields.Char(string='Type')
    equipment_id = fields.Many2one('safety.master', compute='_get_equipment', string='Equipment', store=True)
...
class MailActivity(models.Model):
    _name = 'mail.activity'
    _inherit = ['mail.activity', 'hazard.mail.activity', 'mail.thread']
    ...
    equipment_id = fields.Many2one('safety.master', compute='_get_equipment', string='Equipment', store=True)
    equipment_ids = fields.Many2many(comodel_name='safety.master', relation='mail_activity_maintenance_equipment_rel',
    ...
    equipment_protocol_id = fields.Many2one('hazard.protocol', string="Protocol", ondelete="set null")
    ...
    gefaehrdunsfaktor_ids = fields.One2many('hazard.types', 'name', string="Gefährdungsfaktor Gruppe", store=True)
    gefaehrdunsfaktor_betriebsanweisun_ids = fields.One2many('hazard.types', 'mail_activity_id')
    ...
    def _compute_activities_type(self):
        for rec in self:
            domain = [('id', '!=',  rec.env.ref("safety_master.mail_activity_data_betriebsanweisung").id)]
            if rec.equipment_id and rec.equipment_id.category_id and rec.equipment_id.category_id in [rec.env.ref("safety_master.equipment_hebebuhne"), rec.env.ref("safety_master.equipment_tore"), rec.env.ref("safety_master.equipment_bremsprufstand")]:
                domain = []
...
#     def _compute_activities_type(self):
#         for rec in self:
#             domain = [('id', '!=',  rec.env.ref("safety_master.mail_activity_data_betriebsanweisung_gefahrstoffe").id)]
#             if rec.equipment_id and rec.equipment_id.category_id and rec.equipment_id.category_id in [rec.env.ref("safety_master.equipment_arbeitsstoffe"),]:
#                 domain = []
#             rec.mail_activity_type_ids = rec.env["mail.activity.type"].search(domain).ids
...
#     def _compute_activities_type(self):
#         for rec in self:
#             domain = [('id', '!=',  rec.env.ref("safety_master.mail_activity_data_betriebsanweisung_psa").id)]
#             if rec.equipment_id and rec.equipment_id.category_id and rec.equipment_id.category_id in [rec.env.ref("safety_master.equipment_psa")]:
#                 domain = []
#             rec.mail_activity_type_ids = rec.env["mail.activity.type"].search(domain).ids
    ...
    def _get_equipment(self):
        for activity in self:
            ...
            if activity.res_model == 'safety.master':
    ...
    def action_generate_history_vals(self):
        if self.equipment_test_type and self.category_id and self.equipment_test_type == 'uvv' and self.category_id == self.env.ref("safety_master.equipment_hebebuhne"):
        ...
        if self.equipment_test_type == 'routine_test' and self.category_id == self.env.ref(
                "safety_master.equipment_bremsprufstand"):
        ...
        history3 = self.env['hazard.history'].create(history_vals3)
    ...
    def unlink(self):
        maintenance = self.filtered(lambda rec: rec.res_model == 'safety.master' and rec.active)
        records = self
        if maintenance:
...
#             if self.equipment_test_type == 'el_test':
#                 report = self.env.ref('safety_master.wepelo_equipment_protocol').render_qweb_pdf(protocol.ids[0])
#             if self.equipment_test_type == 'maintenance' and self.exhaust_measuring_device in ['diesel', 'petrol']:
#                 report = self.env.ref('safety_master.wepelo_equipment_protocol_maintenance').render_qweb_pdf(protocol.ids[0])
...
#             self.equipment_id.message_post(body=_('%s Completed (originally assigned to %s)') % (self.activity_type_id.name, self.user_id.name,))
    ...
    def create(self, vals):
        ...
        if res:
            ...
            if res.equipment_test_type == 'uvv' and res.category_id == self.env.ref("safety_master.equipment_hebebuhne"):
                test_specification_data = []
            ...
            if res.equipment_test_type == 'routine_test' and res.category_id == self.env.ref("safety_master.equipment_bremsprufstand"):
                measuring_data = []
                max_difference_data = []
                measuring_data.append(
            ...
            if res.equipment_test_type == 'betriebsanweisung':
                last_activity_gefahrenquelle = self.search(
                    [("equipment_id", '=', res.equipment_id.id), ('activity_type_id', '=', self.env.ref("safety_master.mail_activity_data_calibration_ei").id)],
                    order='id desc', limit=1)
                if last_activity_gefahrenquelle:
                    for gefaehrdunsfaktor in last_activity_gefahrenquelle.gefaehrdunsfaktor_ids:
                        gefaehrdunsfaktor_betriebsanweisun = self.env['hazard.types'].create({
    ...
    def action_close_dialog(self):
        ...
        for not_activity_type in not_activity_types:
            if not_activity_type.res_model == 'safety.master':
        ...
        for activity in self:
                ...
                for schedule_activity_type in activity.schedule_activity_type_ids:
                    ...
                    if activity_done.activity_type_id:
                        if activity_done.force_next:
                            ...
                            if self.res_model == 'safety.master':
                                self.unlink()
            ...
            if len(begehung_id_feld_zwei)> 0:
                ...
                schedule_activity_type = self.env['mail.activity.type'].search(
                    [('id', '=',  self.env.ref('safety_master.mail_activity_data_el_test').id)])
                new_act = self.create({
